CREATE OR REPLACE FUNCTION gere_quantite_produit_insertion() RETURNS TRIGGER language ’plpgsql’ AS
$BODY$
DECLARE
    produit_qte CURSOR FOR SELECT commande_en_ordre.id_commande, comporte.id_produit, comporte.quantite, quai_produit.qte 
    ...
    INNER JOIN quai_produit ON (produit.id_produit=quai_produit.id_produit)
...
BEGIN
    ...
    OPEN produit_qte;
    LOOP 
        FETCH produit_qte INTO commande_id, produit_id, quantite_commande, qte_quai;
		EXIT WHEN NOT FOUND;
        UPDATE quai_produit 
...
CREATE TRIGGER gere_quantite_produit
AFTER INSERT
ON ordre_mission
FOR EACH ROW EXECUTE PROCEDURE gere_quantite_produit_insertion() ;


/* For updating  the qte of a product */
...
BEGIN
    IF OLD.quantite != NEW.quantite THEN 
        IF OLD.id_commande IN (SELECt id_commande FROM commande_en_ordre) THEN 
            ...
            WHERE id_quai = quai_id AND OLD.id_produit = quai_produit.id_produit;
        END IF;
    END IF;
...
/* TRIGGER pour la mise à jour d'une livraison */
CREATE OR REPLACE FUNCTION gere_quantite_produit_update() RETURNS TRIGGER language 'plpgsql' AS
$BODY$
DECLARE
quai_id INTEGER;
